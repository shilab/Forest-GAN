...
def calculate_activation_statistics_from_files(files, sess, batch_size=256, verbose=False):
    """Calculation of the statistics used by the FID.
    Params:
    -- files      : list of paths to image files. Images need to have same dimensions for all files.
    -- sess        : current session
...
def check_or_download_inception(inception_path):
    ...
    inception_def = os.path.join(inception_path, "fid_score")
    model_path = pathlib.Path(inception_def)
    model_file = model_path / 'classify_image_graph_def.pb'
    if not model_file.exists():
        inception_net = os.path.join(inception_path, "inception-2015-12-05.tgz")
    
        import tarfile
        with tarfile.open(inception_net, mode='r') as f:
            f.extract('classify_image_graph_def.pb', inception_def)
